<template>
  <el-container class="flex layout-wrap">
    <el-header class="flex layout-header">
      <div class="flex-auto">
        <router-link :to="{name: routeConfig.projectList}"
                     class="logo">
          <img src="../../../../assets/images/logo.svg"
               alt="" />
        </router-link>
        <h1 class="title ml-1">集成项目交付管理平台</h1>
      </div>
      <div class="flex-item ml-5">
        <nav class="nav-wrap">
          <router-link class="link"
                       :to="{name: routeConfig.projectList}">
            项目管理
          </router-link>
          <router-link class="link"
                       v-if="userinfo.pmo"
                       :to="{name: routeConfig.colonyList}">
            项目集群管理
          </router-link>
          <router-link class="link"
                       v-if="userinfo.pmo"
                       :to="{name: routeConfig.classifyHome}">
            项目类别管理
          </router-link>
          <router-link class="link"
                       v-if="userinfo.pmo"
                       :to="{name: routeConfig.sysHome}">
            项目角色权限管理
          </router-link>
        </nav>
      </div>
...
export default {
  ...
  async beforeRouteEnter(to,from,next){
    await store.dispatch('ISDPOld/project/getProjectDetail', {
      id: to.query.id
    });
    next();
  }
