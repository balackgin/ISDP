<template>
  <div class="riskBackDialog commonDialog">
    <el-dialog
      title="退回"
      :visible.sync="riskBackVisible"
      width="540px"
      :before-close="handleClose"
      :close-on-click-modal="false"
    >
      <div class="riskBackDialog_content">
        <span class="riskBackDialog_content_label">退回原因：</span>
        <el-input
...
<script>
export default {
  props: ['riskBackVisible', 'sendBackRisk'],
  components: {},
  data() {
    return {
      message: ''
  ...
  methods: {
    handleClose() {
      this.$emit('update:riskBackVisible', false);
    },
    saveRiskproblem() {
      if (!this.message) {
        ...
        this.$emit('update:riskBackVisible', false);
        this.sendBackRisk(this.message);
      }
    }
  }
};
